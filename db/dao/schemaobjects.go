...
func GetSchemaObjectsToProcess(conn *sqlx.DB, tenantID, schemaObjectID int, schemaType SchemaType) (s model.SchemaObjectToProcesses, err error) {
	const query = `
		SELECT v.id, v.schema_id, v.schema_name, v.tenant_id, t."name" AS tenant_name, v."type", v.sf_object_id, v.sf_object_name, v.doc_fields, v."sequence", v.filter, v.raw_command, v.sf_last_modified_date
