...
func GetDeviceDataTables(conn *sqlx.DB, tid int, execID int64) (t []*model.DeviceTableField, err error) {
	query := `SELECT o.id AS sf_object_id, 
			   ...
			   WHERE o.sfa_name IN (SELECT DISTINCT table_name FROM public.device_data WHERE so.tenant_id = $1 AND is_active = TRUE AND execution_id = $2 AND so.is_active = TRUE);`
